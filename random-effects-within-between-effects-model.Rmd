...
This example shows how to address the issue of correlating predictors (fixed effects) and group (random) effects, especially in panel data, for mixed models (so called Gauss-Markov-assumption). In particular in econometrics, fixed-effects models are the gold-standard for these cases, however, it often  makes no sense to consider group-effects as "fixed" over a long time period. Apart from this, there are more shortcomings of FE-models as well, see [@bell_fixed_2018] and [@bell_understanding_2018].

The following equations and discussion on FE vs. RE is based on [@bell_fixed_2018]. Further  discussion on FE vs. RE, also at the end of this document, refer to [@gelman_data_2007] and [@bafumi_fitting_2006].

The solution to the critics from "FE-modelers" is simple: If you include a group mean of your variables in a random effects model,it will give the same answer as a fixed effects model (see second table very below, and [@bell_understanding_2018] as reference). This is why FE-modelers often call this type of RE-model also a "kind of" FE-model. However,
...
This code snippet shows how to translate the Equations from [@bell_fixed_2018] into R-code, using `lmer()` from the **lme4**-package.
...
Eq. 10 suggests allowing the "within-effect" (de-meaned) vary across individuals, that's why `x_tv_dm` is added as random slope as well. Since this leads to an error (number of observations <= number of random effects), the check for nobs vs. re is ignored here.
...
Yet, I'm a bit unsure whether `time` or `x_tv_dm` should be used as random-slope in this simpler model. I think `m3` is better here. Again, the estimate of `x_tv_dm` indicates the within-subject effect, while the estimate of `x_tv_gm` indicates the between-subject effect.
...
Here we compare the "classical" RE-model, the complex REWB-model, the "simple" REWB-model and the Mundlak-Model.
