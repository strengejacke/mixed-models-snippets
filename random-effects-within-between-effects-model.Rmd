...
The [panelr-package](https://panelr.jacob-long.com/) provides functions to fit models similar to those suggested by Bell et al. 2018, especially the "simple REWB model" (`m4`) and the Mundlak-model (`m5`).
...
mp4 <- wbm(QoL ~ x_tv | age + z1_ti + z2_ti | (time | ID), data = pd)

# the Mundlak model
mp5 <- wbm(QoL ~ x_tv | age + z1_ti + z2_ti | (time | ID), data = pd, model = "contextual")

tab <- tidy(mp4) %>% 
  bind_cols(tidy(mp5)) %>% 
  select(-statistic, -statistic1, -group1, -term1) %>% 
  round_num(2)

# print models side-by-side
tab_df(
  tab[, c(5, 1, 2, 3, 4, 6, 7, 8)],
  col.header = c(
    "Predictors", "Group", "Est (simple REWB)", "SE (simple REWB)", "p (simple REWB)",
