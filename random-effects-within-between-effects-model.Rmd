...
## The violation of the Gauss-Markov-assumptions in RE-models

This example shows how to address the issue of correlating predictors (fixed effects) and group (random) effects, especially in panel data, for mixed models (one of the Gauss-Markov-assumptions). In particular in econometrics, fixed-effects models are considered the gold-standard for these cases, however, it often makes no sense to consider group-effects as "fixed" over a long time period. Apart from this, there are more shortcomings of FE-models as well, see [@bell_fixed_2018] and [@bell_understanding_2018].
...
The [panelr-package](https://panelr.jacob-long.com/) provides functions to fit models similar to those suggested by Bell et al. 2018, especially the "simple REWB model" (`m4`) and the Mundlak-model (`m5`). For the complex REWB model (`m3`), I needed some slight modification when using `panelr::wbm()`, so the following model `mp3` that mimics the complex REWB model is similar to the above model `m2`.

Here we compare the results from `panelr::wbm()` with our previous models `m3` (complex REWB), `m4` (simple REWB) and `m5` (Mundlak).

**Note:** Since _panelr_ is not yet supported by _sjPlot_, the result from `tidy()` is printed as HTML-table below.
...
mp3 <- wbm(QoL ~ x_tv | age + z1_ti + z2_ti + time  | (time + x_tv | ID), 
           data = pd, 
           control = lmerControl(check.nobs.vs.nRE = "ignore"))

# the simple REWB-model
mp4 <- wbm(QoL ~ x_tv | age + z1_ti + z2_ti + time | (time | ID), data = pd)

# the Mundlak model
mp5 <- wbm(QoL ~ x_tv | age + z1_ti + z2_ti + time | (time | ID), data = pd, model = "contextual")

tab <- tidy(mp3) %>% 
  left_join(tidy(mp4), by = "term") %>% 
  left_join(tidy(mp5), by = "term") %>% 
  select(-statistic, -statistic.x, -statistic.y, -group, -group.x, -group.y) %>% 
  round_num(2)

# print models side-by-side
tab_df(
  tab[1:7, c(4, 1, 2, 3, 5:10)],
  col.header = c(
    "Predictors", "Est (complex REWB)", "SE (complex REWB)", "p (complex REWB)",
    "Est (simple REWB)", "SE (simple REWB)", "p (simple REWB)",
    "Est (Mundlak)", "SE (Mundlak)", "p (Mundlak)"
  )
)
